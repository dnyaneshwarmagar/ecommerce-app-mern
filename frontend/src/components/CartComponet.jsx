...
const CartComponet = ({ cartItems, order, changeQuantity }) => {
  const total = order.items.reduce(
    (total, item) => total + item.price * item.quantity,
    0
  );
  const discount = (total * order.discount_in_percent) / 100;
  const finalTotal = total - discount + order.shipping_charges;
  return (
      ...
      <div className="d-flex flex-row align-items-start">
        ...
        <div className="col-4 order p-3 m-2">
          ...
          <div className="d-flex flex-row justify-content-between p-2">
            ...
            <span className="billing-cost">${total}</span>
          </div>
          <div className="d-flex flex-row justify-content-between p-2">
